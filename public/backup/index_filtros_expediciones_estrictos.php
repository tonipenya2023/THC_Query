...
function render_advanced(): void
{
    ...
    // selector de lineas por pagina eliminado: siempre 20
...
function render_expeditions(): void
{
    ...
    if ($playerNames !== []) {
        ...
        foreach ($playerNames as $idx => $name) {
            $ph = ':player_name_' . $idx;
            // El filtro de Jugador en Expediciones debe filtrar el jugador de la expedición,
            // no jugadores de muertes asociadas.
            $parts[] = 'e.player_name = ' . $ph;
            $params[$ph] = $name;
        }
    ...
    if ($reserveNames !== []) {
        ...
        foreach ($reserveNames as $idx => $name) {
            ...
            $parts[] = 'LOWER(TRIM(e.reserve_name)) = LOWER(TRIM(' . $ph . '))';
            $params[$ph] = $name;
        }
        $where[] = '(' . implode(' OR ', $parts) . ')';
    }
...
function render_classifications_history(): void
{
    ...
    echo '<button type="button" class="btn-reset-cols" data-default-cols="' . h(implode(',', $defaultCols)) . '">Restablecer</button>';
    echo '</div></details>';
    echo '<select <option value="50"' . ($pageSize === 50 ? ' selected' : '') . '>50 filas</option><option value="100"' . ($pageSize === 100 ? ' selected' : '') . '>100 filas</option><option value="200"' . ($pageSize === 200 ? ' selected' : '') . '>200 filas</option><option value="500"' . ($pageSize === 500 ? ' selected' : '') . '>500 filas</option></select>';
    echo '<button type="submit">Filtrar</button>';
    echo '<button type="submit" name="export_csv" value="1">Exportar CSV</button>';
...
function render_kill_url_scrape_status(): void
{
    ...
    echo '<button type="button" class="btn-reset-cols" data-default-cols="' . h(implode(',', $defaultCols)) . '">Restablecer</button>';
    echo '</div></details>';
    echo '<select <option value="50"' . ($pageSize === 50 ? ' selected' : '') . '>50 filas</option><option value="100"' . ($pageSize === 100 ? ' selected' : '') . '>100 filas</option><option value="200"' . ($pageSize === 200 ? ' selected' : '') . '>200 filas</option></select>';
    echo '<button type="submit">Filtrar</button>';
    echo '<button type="submit" name="export" value="csv">Exportar CSV</button>';
