...
function parseKillDetailPayload(array $payload): array
{
    ...
    if (preg_match('~var killData = (\{.*?\})\s*,\s*strs =~s', $html, $m) === 1) {
        ...
        if (is_array($json)) {
            if ($parsedHitRows !== []) {
                $json['hits'] = mergeParsedHitRowsIntoHits(
                    isset($json['hits']) && is_array($json['hits']) ? $json['hits'] : [],
                    $parsedHitRows
                );
            }
            $out['kill_data'] = $json;
    ...
    if ($out['kill_data'] === null && $parsedHitRows !== []) {
        $out['kill_data'] = ['hits' => $parsedHitRows];
    }
 ...
 */
/**
 * Mezcla las filas visibles de la tabla de disparos dentro de kill_data['hits'].
 *
 * @param array<int,mixed> $rawHits
 * @param array<int,array<string,string|null>> $parsedHitRows
 * @return array<int,array<string,mixed>>
 */
function mergeParsedHitRowsIntoHits(array $rawHits, array $parsedHitRows): array
{
    $hits = [];
    $max = max(count($rawHits), count($parsedHitRows));

    for ($i = 0; $i < $max; $i++) {
        $raw = $rawHits[$i] ?? [];
        $parsed = $parsedHitRows[$i] ?? [];
        ...
        // Mantiene los campos originales de killData.hits y añade/sobrescribe
        // los textos ya resueltos desde la tabla HTML: arma, visor, munición, distancia, etc.
        $hits[] = array_merge($raw, $parsed);
    }
